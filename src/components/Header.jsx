...
const Header = () => {
  ...
  const{hideSideBar,setHideSiderBar}=useContext(FirstContext)
  ...
  return (
    <>
      <div className="header sticky top-0 left-0 grid  grid-cols-3 w-[100%]  items-center bg-black py-4 px-7 text-white">
        ...
        <div className="youtub-icon flex  items-center gap-4 cursor-pointer col">
          <GiHamburgerMenu size={24} onClick={()=>setHideSiderBar(!hideSideBar)}/>
         <img src={img} alt="youtube-logo"  className="h-7 w-27" onClick={()=>navigate('/')}/>
        </div>
        ...
        <AiOutlineVideoCameraAdd   size={26}/>
        <IoIosNotificationsOutline size={26} />
        <img src={randomAvatar} alt="random-avatar"  className="h-10 rounded-full"/>
      </div>
